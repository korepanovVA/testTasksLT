...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<Float> dataInputFile1 = new ArrayList<>();

        while(scanner.hasNextLine()) {
            dataInputFile1.add(scanner.nextFloat());
        }
        inputFile.close();

        inputFile = new FileReader(args[1]);
        scanner = new Scanner(inputFile);
        ArrayList<Float> dataInputFile2 = new ArrayList<>();

        while (scanner.hasNextLine()){
            dataInputFile2.add(scanner.nextFloat());
        }
        inputFile.close();

        for(int i = 0; i < dataInputFile2.size(); i++){
            System.out.println(dataInputFile2.get(i));
        }
